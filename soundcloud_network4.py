...
class SoundCloudNetwork():
    def __init__(self):
        ...
        # 0=num_tracks, 1=num_followers, 2=num_following, 3=artist_city, 4=latitude, 5=longitude
        self.attrs = [{}, {}, {}, {}, {}, {}]
        self.correct_attrs = [{}, {}, {}, {}, {}, {}]
        self.artist_id_dict = {}
        self.nodes = []
    ...
    def print_large_network(self):
        ...
        self.gen_edge_list(filename="soundcloud_edge_list4.txt")
        self.gen_attr(filename="soundcloud_attr4.txt")
        self.gen_network(edge_list="soundcloud_edge_list4.txt", attr="soundcloud_attr4.txt")
            ...
            for key in keys:
                ...
                for follower in followers:
                    ...
                    if follower in self.artists.keys():
                        ...
                        self.nodes.append(self.artists[artist]['id'])
    ...
    def gen_attr(self, filename="soundcloud_attr.txt"):
        ...
        for artist in self.artists.keys():
            if self.artists[artist]['id'] in self.nodes:
    ...
    # Function used to check to see number of edges between non-artists/artists and artists/artists
    def analyze_edges(self):
        u_a = 0
        a_a = 0
        u_u = 0
        for edge in self.network.edges():
            if self.try_key(self.artist_id_dict[int(edge[0])], 'num_tracks') == 0:
                if self.try_key(self.artist_id_dict[int(edge[1])], 'num_tracks')  == 0:
                    u_u += 1
                else:
                    u_a += 1
            else:
                if self.try_key(self.artist_id_dict[int(edge[1])], 'num_tracks')  == 0:
                    ...
                    a_a += 1
        print("Users to Users: ", u_u)
        print("Users to Artists: ", u_a)
        print("Artists to Artists: ", a_a)
        print("Sum: ", u_u + u_a + a_a)
        print("Total Edges: ", len(self.network.edges()))
...
def main():
    ...
    #sc.clean_artist_lr()
    #sc.gen_edge_list(filename="soundcloud_edge_list4.txt")
    #sc.gen_attr(filename="soundcloud_attr4.txt")
    sc.gen_network(edge_list="soundcloud_edge_list4.txt", attr="soundcloud_attr4.txt")
    sc.analyze_edges()
    # sc.print_network()
    # sc.triangle_scores()
    # sc.clustering_coef()
