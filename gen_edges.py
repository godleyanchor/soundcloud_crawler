...
class Crawl():
    ...
    def try_key(self, artist, key):
        try:
            ...
            if key == "num_tracks" or key == "num_followers":
...
def main(depth=3):
        ...
        # Initialize artists
        if load:
            soundCrawler.load_artists("artists_large_connected.p")
        else:
            soundCrawler.crawl_top_artists(url_top)

        # Start crawling
        if load:
            artist_list = soundCrawler.load_new_artists("new_artists_large_connected.p")[100:]
        else:
        ...
        for i in range(depth):
            ...
            for name in artist_list:
                ...
                print("-------------------------\nArtists dumped to artists_large_connected.p.\n-------------------------")
                soundCrawler.dump_artists("artists_large_connected.p")
                soundCrawler.dump_new_artists(new_artist_list, "new_artists_large_connected.p")
            print("-------------------------\nNew Depth\n-------------------------")
            soundCrawler.dump_artists("artists_large_connected.p")
            soundCrawler.dump_new_artists(new_artist_list, "new_artists_large_connected.p")
            try:
